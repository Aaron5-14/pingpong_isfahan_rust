use std::{collections::VecDeque, f32::consts::PI, process::exit};
...
impl<'a> Frame<'a> {
    ...
    fn new(balls: Vec<Ball<'a>>, paddles: (Paddle<'a>, Paddle<'a>)) -> Self {
        Frame {
            balls,
            paddles,
            powerup: None,
        ...
        match &mut game_state {
            ...
            GameState::Menu(sel) => {
                ...
                for i in 0..menu_options.len() {
                    ...
                    let color = if i == select { BLACK } else { WHITE };
            ...
            GameState::Replay => {
                if replay_frames.len() != 0 {
                    if let Some(frm) = replay_frames.pop_front() {
                        frm.draw();
                    }
                } else {
